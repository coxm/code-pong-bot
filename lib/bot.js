...
const messageCommands = {
  async hype(client, args, {recipient}) {
    ...
    try {
      ...
      message = onError(err, {uri, args, command: 'hype'});
    }
  ...
  async panic(client, args, {recipient}) {
    ...
    const uri = tpl.renderString(env.api.countdown);
    let message = '';
    try {
      ...
      message = onError(err, {uri, args, command: 'panic'});
    }
    client.say(recipient, message);
  },
  ...
  roll(client, args, {recipient}) {
    ...
    for (let numberString of query.split(/[^0-9]+/)) {
